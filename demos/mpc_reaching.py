...
endeff_translation = np.array([0.4, 0.4, 0.3])  #  move endeff +10 cm along x in WORLD frame
...
# Solve
ddp.solve(xs_init, us_init, 100, False)
...
for i in range(sim_data['N_sim']):
    ...
    if (i % int(sim_params['sim_freq'] / sim_params['mpc_freq']) == 0):
        ...
        ddp.solve(xs_init, us_init, ocp_params['maxiter'], False)
        ...
        sim_data['ctrl_ref'][mpc_cycle, :] = pin_utils.get_u_grav(q,
                                                                  ddp.problem.runningModels[0].differential.pinocchio,
                                                                  ocp_params['armature'])
        sim_data['state_ref'][mpc_cycle, :] = ddp.problem.runningModels[0].differential.costs.costs[
            'stateReg'].cost.residual.reference
        sim_data['lin_pos_ee_ref'][mpc_cycle, :] = ddp.problem.runningModels[0].differential.costs.costs[
            'translation'].cost.residual.reference
...
mpc_utils.plot_mpc_results(plot_data, which_plots=['all'], PLOT_PREDICTIONS=True,
