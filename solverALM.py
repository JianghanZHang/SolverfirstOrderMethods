...
class SolverALM(SolverAbstract):
    ...
    def backwardPass(self):
        ...
        self.dLdx[-1, :] = self.problem.terminalData.Lx + (-self.lambdas[-1, :] + self.miu * self.constraints[-1]).T @ (-np.eye(self.problem.terminalData.Fx.shape[0]))
        ...
        for t, (model, data) in rev_enumerate(zip(self.problem.runningModels, self.problem.runningDatas)):
            ...
            self.dLdu[t, :] = data.Lu + (-self.lambdas[t+1, :] + self.miu * self.constraints[t+1, :]).T @ data.Fu
            self.dLdx[t, :] = data.Lx + (-self.lambdas[t+1, :] + self.miu * self.constraints[t+1, :]).T @ data.Fx + (-self.lambdas[t-1, :] + self.miu * self.constraints[t, :]).T @ -np.eye(data.Fx.shape[0]) 
    ...
    def forwardPass(self, alpha):
        ...
        for t, (model, data) in enumerate(zip(self.problem.runningModels, self.problem.runningDatas)):
            ...
            cost_try += data.cost
            self.constraints[t+1] = data.xnext - self.xs[t+1]  # c[t](x[t], u[t], x[t+1]) = f(x[t], u[t]) - x[t+1]
    ...
    def updatingALM(self):
        ...
        # pdb.set_trace()
        self.lambdas[1:] -= self.miu * self.constraints[1:]
