...
class SolverMSls(SolverAbstract):
    ...
    def forwardPass(self, alpha, i):
        ...
        if self.bias_correction:
            ...
            m_x_corrected = self.m_x / (1 - self.Beta1 ** (i + 2))
            v_x_corrected = self.v_x / (1 - self.Beta2 ** (i + 2))
        else:
            m_corrected = self.m
            v_corrected = self.v

        self.update_u = -m_corrected / (np.sqrt(v_corrected) + self.eps)
        # self.update_u = -self.dJdu
        self.update_x = -m_x_corrected / (np.sqrt(v_x_corrected) + self.eps)
        self.update_x[0] = 0
        ...
        self.accumulated_update_x += alpha * self.update_x
        self.accumulated_update_x[0] = 0
        # if i == 5:
        #     pdb.set_trace()
        self.us_try = list(us_try)
        ...
        for t, (model, data) in enumerate(zip(self.problem.runningModels, self.problem.runningDatas)):
            ...
            if i % interval == 0:
                self.update_x[t+1] = (data.Fx @ self.update_x[t] + data.Fu @ self.accumulated_update_u[t] + self.gap[t+1])
        ...
        if i % interval == 0:
            self.accumulated_update_u *= 0
            self.accumulated_update_x *= 0
    ...
    def allocateData(self):
        ...
        self.accumulated_update_u = np.array([np.zeros(m.nu) for m in self.problem.runningModels])
        self.accumulated_update_x = np.array([np.zeros(m.state.ndx) for m in self.models()])
        ...
        self.m_x = np.zeros_like(self.dJdx)
        self.v_x = np.zeros_like(self.dJdx)
