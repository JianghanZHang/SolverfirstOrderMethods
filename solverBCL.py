...
class SolverALM(SolverAbstract):
    ...
    def forwardPass(self, alpha):
        ...
        #     m_corrected = self.m
        #     v_corrected = self.v
        update_u = self.m_u / (np.sqrt(self.v_u) + self.eps)
        update_x = self.m_x / (np.sqrt(self.v_x) + self.eps)
        # update_u = self.dLdu
        # update_x = self.dLdx
        us = np.array(self.us)
        xs = np.array(self.xs)
        us_try = us - alpha * update_u
        xs_try = xs.copy()
        # import pdb; pdb.set_trace()
        xs_try[1:] -= alpha * update_x[1:]
        # print(f"xs_try:{xs_try}")
        # print(f"us_try:{us_try}")
        ...
        for t, (model, data) in enumerate(zip(self.problem.runningModels, self.problem.runningDatas)):
            ...
            cost_try += data.cost
            self.constraints[t] = data.xnext - self.xs_try[t+1]  # f(x[t], u[t]) - x[t+1]
            cost_try += (self.lambdas[t, :] + .5 * self.miu * self.constraints[t]).T @ self.constraints[t] 
    ...
    def solve(self, init_xs=None, init_us=None, maxOuterIter=10, maxInnerIter=1000, isFeasible=False):
        ...
        while self.outer_iter < maxOuterIter and self.inner_iter < maxInnerIter:
            ...
            self.checkUpdatingandConvergence()
    ...
    def allocateData(self):
        ...
        self.lambdas = np.array([np.zeros(m.state.nx) for m in self.problem.runningModels])
        ...
        self.constraintThreadshold = CONFIG["constraintThreshold"]
        self.gradientThreshold = CONFIG["gradientThreshold"]
        self.constraints = [np.zeros(m.state.nx) for m in self.models()]
        self.dynInf = 0.
        self.alpha_p = 0.
